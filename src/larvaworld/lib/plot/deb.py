...
def plot_food_amount(filt_amount: bool = False, scaled: bool = False, **kwargs: Any) -> Any:
    ...
    for lab, d, c in P.data_palette:
        ...
        if filt_amount:
            sos = signal.butter(
                N=1, Wn=0.1, btype="lowpass", analog=False, fs=1 / P.dt, output="sos"
            )
            dst_m = dst_m.diff()
            dst_m.iloc[0] = 0
...
def plot_debs(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        save_to: Directory for saving. Defaults to simulation directory
        mode: Variable set to plot ('full', 'energy', 'growth', 'feeding', etc.). Defaults to 'full'
        roversVSsitters: Color code by rover/sitter phenotype. Defaults to False
        include_egg: Include egg stage in timeline. Defaults to True
        time_unit: Time unit for x-axis ('hours', 'minutes', 'seconds'). Defaults to 'hours'
        sim_only: Show only simulation period. Defaults to False
        force_ymin: Force minimum y-value. Defaults to None
        color_epoch_quality: Color epochs by quality. Defaults to True
        datasets: Dataset list for loading DEB data. Required if deb_dicts is None
        labels: Custom labels. Defaults to None
    ...
    if name is None:
        name = "debs"
    if save_to is None:
        save_to = f"{SIM_DIR}/deb_runs"
    if deb_dicts is None:
        deb_dicts = util.flatten_list([d.load_dicts("deb") for d in datasets])
    Ndebs = len(deb_dicts)
    ...
    ylabels0 = [
        ...
        "food absorption $(mg)$",
        "faeces $(mg)$",
        "food not digested $(mg)$",
        "product not absorbed $(mg)$",
        "faeces fraction",
        "absorption efficiency",
        "fraction not digested",
        "gut occupancy",
        r"[p$_{A}^{deb}$] $(microJ/cm^3)$",
        r"[p$_{A}^{sim}$] $(microJ/cm^3)$",
        r"[p$_{A}^{gut}$] $(microJ/cm^3)$",
        # r'[p$_{A}^{deb}$] $(\mu J/cm^3)$', r'[p$_{A}^{sim}$] $(\mu J/cm^3)$',r'[p$_{A}^{gut}$] $(\mu J/cm^3)$',
        r"f $^{gut}$ $(-)$",
        r"$\Delta$p$_{A}^{gut}$ $(-)$",
        r"Food in gut $(C-moles)$",
        r"Product in gut $(C-moles)$",
        r"Product absorbed $(C-mmoles)$",
        r"Active enzyme amount in gut $(-)$",
        r"Available carrier amount in gut surface $(-)$",
        r"Available carrier ratio in gut surface $(-)$",
        r"Active enzyme ratio in gut surface $(-)$",
        r"Food VS Product ratio in gut $(-)$",
        r"Ratio of Food in gut $(-)$",
        r"Ratio of Product in gut $(-)$",
    ...
    P = plot.AutoBasePlot(
        name=name,
        save_to=save_to,
        build_kws={"Nrows": Npars, "sharex": True, "sharey": sharey, "w": 20, "h": 6},
        **kwargs,
    )
    ...
    for jj, (d, id, c) in enumerate(zip(deb_dicts, ids, cols)):
        ...
        for j, (l, yl) in enumerate(zip(labels, ylabels)):
            ...
            for (st0, st1), qq in zip(epochs, epoch_qs):
                q_col = (
                    util.col_range(qq, low=(255, 0, 0), high=(255, 255, 255))
                    if color_epoch_quality
                    else c
                )
            ...
            if l == "f" or mode == "fs":
                ax.axhline(
                    np.nanmean(ppp), color=c, alpha=0.6, linestyle="dashed", linewidth=2
                )
    ...
    P.conf_ax(
        j,
        xlab=f"time $({time_unit})$",
        xMaxN=5,
        xlim=(0, np.max(max_ages) if sim_only else None),
    )
    ...
    plot.dataset_legend(
        leg_ids, leg_cols, ax=P.axs[0], loc="upper left", fontsize=20, prop={"size": 15}
    )
...
def plot_EEB_vs_food_quality(
    ...
    **kwargs: Any,
    ...
    if name is None:
        name = "EEB_vs_food_quality"
    qs = np.arange(0.01, 1, 0.01)

    P = plot.AutoBasePlot(
        name=name,
        build_kws={"Nrows": 3, "Ncols": len(refIDs), "w": 10, "h": 7},
        **kwargs,
    )
    ...
    for i, refID in enumerate(refIDs):
        ...
        for col, species in zip(util.N_colors(len(species_list)), species_list):
            ss = []
            EEBs = []
            cc = {"color": col, "label": species, "marker": "."}
            for q in qs:
