...
class GeoLarvaDataset(BaseLarvaDataset, mpd.TrajectoryCollection):
    ...
    def init_gdf(self, step: pd.DataFrame, dt: float) -> "gpd.GeoDataFrame":  # type: ignore
        ...
        if "xy" not in step.columns:
            assert util.cols_exist(["x", "y"], step)
    ...
    def init_mpd(self, step: pd.DataFrame, dt: float) -> None:
        ...
        for tr in self:
            ...
            tr.df[["x", "y"]] = tr.df[["x", "y"]].astype(
                {p: self.spatial_pint_unit for p in ["x", "y"]}
            )
    ...
    def get_means(self, pars=None, ks=None):
        ...
        valid = [p for p in pars if self.cols_exist_in_all_traj([p])]
        for p in valid:
            p_mu = util.nam.mean(p)
            self.endpoint_data[p_mu] = {tr.id: np.mean(tr.df[p].values) for tr in self}
            self.endpoint_data[p_mu] = self.endpoint_data[p_mu].astype(
                {p_mu: self.dtypes[p]}
            )
    ...
    def detect_pauses(self, max_scaled_diameter=0.3, min_duration=timedelta(seconds=1)):
        ...
        for traj in self:
            ...
            kws = {
                "max_diameter": max_scaled_diameter * l,
    ...
    def get_complete_segments_between(self, t1, t2):
        ...
        return [
            tr.df[self.time_to_datetime(t1) : self.time_to_datetime(t2)]
            for tr, e in self.iter
    ...
    def build_endpoint_data(self, e: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        if cum_d not in e.columns:
            e[cum_d] = {
                traj.id: self.get_length_from_traj_with_nans(traj) for traj in self
            }
        ...
        if t0 not in e.columns and t1 not in e.columns:
            e[t0] = {
                traj.id: mpd.trajectory.to_unixtime(traj.df.index.min())
                for traj in self
            }
            e[t1] = {
                traj.id: mpd.trajectory.to_unixtime(traj.df.index.max())
                for traj in self
            }
        ...
        e[spatial_ps] = e[spatial_ps].astype(
            {p: self.spatial_pint_unit for p in spatial_ps}
        )
        temporal_ps = [t0, t1] + [cum_t]
        e[temporal_ps] = e[temporal_ps].astype(
            {p: self.temporal_pint_unit for p in temporal_ps}
        )
        ...
        if self.cols_exist_in_all_traj(xy_flat):
            for tr in self:
                if keep_midline_LineString:
                    tr.df["midline"] = tr.df.apply(
                        lambda r: shp.geometry.LineString(
                            [(r[x], r[y]) for [x, y] in xy_pairs]
                        ),
                        axis=1,
                    )
    ...
    def load_contour(self, drop=True):
        ...
        if self.cols_exist_in_all_traj(xy_flat):
            for tr in self:
                tr.df["contour"] = tr.df.apply(
                    lambda r: shp.geometry.Polygon(
                        [(r[x], r[y]) for [x, y] in xy_pairs]
                    ),
                    axis=1,
                )
    ...
    def set_dtype(self, cols, units):
        ...
        cols_end = util.existing_cols(cols, self.endpoint_data)

        pint_dtypes = dict(zip(cols, units))
        if len(cols_end) > 0:
            self.endpoint_data[cols_end] = self.endpoint_data[cols_end].astype(
                {col: pint_dtypes[col] for col in cols_end}
            )
        for tr in self:
            cols_traj = util.existing_cols(cols, tr.df)
            if len(cols_traj) > 0:
                tr.df[cols_traj] = tr.df[cols_traj].astype(
                    {col: pint_dtypes[col] for col in cols_traj}
                )
    ...
    def match_ids(self):
        ...
        f = util.flatten_list(combos)
        uf = util.unique_list(f)
        assert len(f) == len(uf)
        traj_ids_to_drop = []
        for combo in combos:
        ...
        self.trajectories = [
            tr for tr in self.trajectories if tr.id not in traj_ids_to_drop
        ]
        vprint("**--- Recomputing endpoint metrics -----", verbose)
        ...
        self.endpoint_data = end.sort_index()
        vprint(
            f"**--- Trajectories reduced from {Nids0} to {self.config.N} by the matchIDs algorithm -----",
            verbose,
        )
