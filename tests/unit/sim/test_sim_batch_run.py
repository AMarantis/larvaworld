...
class TestOptimizationOps:
    ...
    def test_check_max_sims_reached(self):
        ...
        with patch('larvaworld.lib.sim.batch_run.vprint') as mock_vprint:
            ops.check(fits)
            mock_vprint.assert_called_once_with("Maximum number of simulations reached. Halting search", 2)
    ...
    def test_check_threshold_reached(self):
        ...
        with patch('larvaworld.lib.sim.batch_run.vprint') as mock_vprint:
            ops.check(fits)
            mock_vprint.assert_called_once_with("Best result reached threshold. Halting search", 2)
    ...
    def test_check_threshold_not_reached(self):
        ...
        with patch('larvaworld.lib.sim.batch_run.vprint') as mock_vprint:
            ops.check(fits)
            mock_vprint.assert_called_once_with("Not reached threshold. Expanding space search", 2)
...
class TestBatchRun:
    ...
    def mock_batch_run(self):
        ...
        with patch('larvaworld.lib.sim.batch_run.ap.Experiment') as mock_ap_experiment, \
             patch('larvaworld.lib.sim.batch_run.reg.generators.SimConfiguration') as mock_sim_conf, \
             patch('larvaworld.lib.sim.batch_run.reg.conf.Exp.expand') as mock_exp_expand:
            ...
            batch_run.figs = {}
            batch_run.par_df = pd.DataFrame({"param1": [1, 2, 3], "fit": [0.1, 0.2, 0.3]})
    ...
    def test_single_sim(self, mock_batch_run):
        ...
        mock_batch_run.exp_conf.update_existingnestdict_by_suffix.return_value = {"param1": 1.0}
    ...
    def test_single_sim_with_variables_removal(self, mock_batch_run):
        ...
        mock_model_instance = Mock()
        mock_model_instance.simulate.return_value = [Mock()]
        mock_model_instance.output = {"test": "output", "variables": {"var1": [1, 2, 3]}}
        mock_batch_run.model.return_value = mock_model_instance
    ...
    def test_default_processing_parameter_not_found(self, mock_batch_run):
        ...
        with pytest.raises(ValueError, match="Could not retrieve fit parameter from dataset"):
    ...
    def test_end(self, mock_batch_run):
        ...
        mock_batch_run.output._combine_pars.return_value = pd.DataFrame({"param1": [1, 2, 3]})
    ...
    def test_plot_results_single_parameter(self, mock_batch_run):
        ...
        with patch('larvaworld.lib.sim.batch_run.plot_2d') as mock_plot_2d, \
             patch.object(type(mock_batch_run), 'plot_dir', new_callable=lambda: property(lambda self: "/test/plots")):
    ...
    def test_plot_results_two_parameters(self, mock_batch_run):
        ...
        with patch('larvaworld.lib.sim.batch_run.plot_3pars') as mock_plot_3pars, \
             patch.object(type(mock_batch_run), 'plot_dir', new_callable=lambda: property(lambda self: "/test/plots")):
    ...
    def test_PI_heatmap(self, mock_batch_run):
        ...
        mock_batch_run.datasets = {
            0: [Mock()], 1: [Mock()], 2: [Mock()]
        }
        ...
        with patch('larvaworld.lib.sim.batch_run.plot_heatmap_PI') as mock_plot_heatmap, \
    ...
    def test_space_search_sample_direct_values(self):
        ...
        with patch('larvaworld.lib.sim.batch_run.ap.Sample') as mock_sample:
            ...
            mock_sample.assert_called_once_with(
                {"param1": 1.0, "param2": "test", "param3": True},
                n=1
            )
